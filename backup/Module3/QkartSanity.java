...
public class QkartSanity {
    ...
    public static Boolean TestCase08(RemoteWebDriver driver) throws InterruptedException {
        ...
        logStatus("Start TestCase", "Test Case 8: ", "DONE");
        ...
        if (!status) {
            logStatus("Step Failure", "User Perform Registration Failed", status ? "PASS" : "FAIL");
            logStatus("End TestCase", "Test Case 8: Mutitab scenario: ", status ? "PASS" : "FAIL");
            takeScreenshot(driver, "failed", "TestCase07");
            return false;
        }
        lastGeneratedUserName = registration.lastGeneratedUsername;
        ...
        homePage.navigateToHome();
        List<String> expectedProductTitles = Arrays.asList("YONEX Smash Badminton Racquet");
        status = homePage.verifyCartContents(expectedProductTitles);
        driver.close();
    ...
    public static Boolean TestCase09(RemoteWebDriver driver) throws InterruptedException {
        ...
        logStatus("Step", "Verify that the url of the current tab does not changed",
                status ? "PASS" : "FAIL");
        Set<String> windowsSet = driver.getWindowHandles();
        ...
        status = privacyPolicyHeader.isDisplayed();
        logStatus("Step", "Verify that the privacy policy content is available: ",
                status ? "PASS" : "FAIL");
        ...
        windowsList = new ArrayList<>(windowsSet);
        driver.switchTo().window(windowsList.get(2));

        WebElement termsOfServicesHeader = driver.findElement(By.xpath("//h2[text()='Terms of Service']"));
        status = termsOfServicesHeader.isDisplayed();
        logStatus("Step", "Verify that the Terms of sevicees content is available: ",
                status ? "PASS" : "FAIL");
        driver.close();
        driver.switchTo().window(windowsList.get(1));
        driver.close();
    ...
    public static Boolean TestCase10(RemoteWebDriver driver) throws InterruptedException {
        ...
        WebElement nameElement = driver.findElement(By.xpath("//input[@placeholder='Email']"));
        WebElement emaiElement = driver.findElement(By.xpath("//input[@placeholder='Name']"));
        WebElement messageElement = driver.findElement(By.xpath("//input[@placeholder='Message']"));
        nameElement.sendKeys("crio user");
        emaiElement.sendKeys(" criouser@gmail.com");
        ...
        contactNow.click();
        status = wait.until(ExpectedConditions.invisibilityOf(contactNow));
        logStatus("test case 10", "Verify contact us link: ",
                status ? "PASS" : "FAIL");
    ...
    public static Boolean TestCase11(RemoteWebDriver driver) throws InterruptedException {
        ...
        List<WebElement> addsElements = driver.findElements(By.xpath("//iframe"));
        status = addsElements.size() == 3;
        logStatus("Step", "Test Case 11:  Check for 3 Advertisement : ",
                    status ? "PASS" : "FAIL");

        for(int i=0 ; i<=1 ; i++){
        driver.switchTo().frame(i);
        WebElement bynowButton =driver.findElement(By.xpath("//button[text()='Buy Now']"));
        bynowButton.click();
        wait.until(ExpectedConditions.urlContains("/checkout"));

        String currentURL = driver.getCurrentUrl();
        status =currentURL.endsWith("/checkout");
        logStatus("Step", "Test Case 11:  buy Now button is not working : ",
                    status ? "PASS" : "FAIL");
