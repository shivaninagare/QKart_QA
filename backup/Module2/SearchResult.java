    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders,
        ...
        try {
            ...
            for(int i=0; i< expectedTableHeaders.size(); i++){
                ...
                int col = i+1;
                WebElement headerElement = driver.findElement(By.xpath("//table/thead/tr/th["+col+"]"));
                String actualTHeader = headerElement.getText();
                if(!actualTHeader.equals(expectedTHeader)){
                    status =false;
                }
            }
            for(int i = 0;i<expectedTableBody.size();i++){
                List<String> completeRow = expectedTableBody.get(i);

                for(int j =0 ;j<completeRow.size();j++){
                    String expectedTBody = completeRow.get(j);
                    int row= i+1;
                    int col = j+1;
                    WebElement tableBodyElement = driver.findElement(By.xpath("//table/tbody/tr["+row+"]/td["+col+"]"));
                    String actualBody = tableBodyElement.getText();
                    if(!actualBody.equals(expectedTBody)){
                        status= false;
                    }
                }
            }
